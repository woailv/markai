...
import (
	...
	"prompttool/internal/pkg/pathutil"
...
func NewFileService(snapshots *snapshot.SnapshotService) *FileService {
...
func (s *FileService) Write(in WriteFileInput) (*WriteFileResult, error) {
	...
	if !created {
		if err := fsattr.ClearReadOnly(abs); err != nil {
...
func (s *FileService) Rename(in RenameInput) (*RenameResult, error) {
	...
	if err := fsattr.ClearReadOnly(src); err != nil {
...
// requireAbs 包装 pathutil.RequireAbs,统一错误前缀。
func requireAbs(p string) (string, error) {
	abs, err := pathutil.RequireAbs(p)
	if err != nil {
