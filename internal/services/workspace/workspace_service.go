...
// recentSvc 用于在切换根目录时记录一条"最近打开",可为 nil 表示不启用该联动。
func NewWorkspaceService(cfg config.WorkspaceConfig, recentSvc *recent.RecentService) *WorkspaceService {
...
// 供 watcher 推送变更事件使用。
func (s *WorkspaceService) SetEmitter(e eventbus.Emitter) {
	s.mu.Lock()
	s.emitter = e
	s.mu.Unlock()
}
...
func (s *WorkspaceService) SetRoot(in SetWorkspaceRootInput) (*WorkspaceRootInfo, error) {
	...
	s.mu.Unlock()
	if startErr == nil && recentSvc != nil {
		// 记录失败不影响主流程,仅当作降级处理。
		_, _ = recentSvc.Record(abs)
	}
...
func listDirEntries(dir string, includeHidden bool, ignoreSet map[string]struct{}) ([]WorkspaceEntry, error) {
	...
	for _, e := range raw {
		...
		hidden := fsattr.IsHidden(full, name)
...
func statEntry(path string) (*WorkspaceEntry, error) {
	...
	return &WorkspaceEntry{
		...
		Parent:    filepath.Dir(path),
		IsDir:     isDir,
		IsHidden:  fsattr.IsHidden(path, name),
		IsSymlink: isSymlink,
		Size:      info.Size(),
		ModTime:   info.ModTime().Unix(),
	}, nil
