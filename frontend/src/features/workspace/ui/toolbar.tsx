...
import { refreshRoot } from "../model/use-workspace-events"
...
interface WorkspaceToolbarProps {
  /**
   * "最近打开" 浮层的内容渲染器。由页面组合层(pages/home)注入,
   * 保持 features/workspace 不反向依赖 widgets/recent-list。
   * 未提供时,"最近打开" 按钮不渲染。
   */
  ...
  return (
    <div className="relative shrink-0">
      ...
      {recentOpen && renderRecent && (
        ...
        >
          <div className="h-[300px]">{renderRecent(() => setRecentOpen(false))}</div>
        </div>
      )}
    </div>
  )
